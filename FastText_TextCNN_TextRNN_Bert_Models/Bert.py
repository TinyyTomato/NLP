...
class Model(nn.Module):
    def __init__(self, vocab):
        ...
        # 打印信息: 使用bert词嵌入作为编码, lstm作为传输的编码器
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.best_dev_f1 <= dev_f1:
                ...
                if self.early_stop == early_stops:
                    logging.info("Eearly stop in epoch %d, best train: %.2f, dev: %.2f" % (epoch - early_stops, self.best_train_f1, self.best_dev_f1))
        ...
        for batch_data in data_iter(self.train_data, train_batch_size, shuffle=True):
            ...
            if batch_idx % log_interval == 0:
                ...
                logging.info('| epoch {:3d} | step {:3d} | batch {:3d}/{:3d} | lr{} | loss {:.4f} | s/batch {:.2f}'.format(
                        epoch, self.step, batch_idx, self.batch_num, lrs, losses / log_interval, elapsed / log_interval))
                losses = 0
                start_time = time.time()
            ...
            logging.info('| epoch {:3d} | score {} | f1 {} | loss {:.4f} | time {:.2f}'
                         .format(epoch, score, f1, overall_losses, during_time))
