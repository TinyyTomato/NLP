...
WordCNNEncoder包括两个embedding层，分别对应batch_inputs1，embedding 层是可学习的，
得到word_embed；batch_inputs2，读取的是外部训练好的词向，因此是不可学习的，得到extword_embed。
所以会分别得到两个词向量，将 2 个词向量相加，得到最终的词向量batch_embed，
其形状是(batch_size * doc_len, sent_len, 100)，
然后添加一个维度，变为(batch_size * doc_len, 1, sent_len, 100)，
对应 Pytorch 里图像的(B, C, H, W)。

2. CNN
然后，分别定义 3 个卷积核，output channel 都是 100 维。
第一个卷积核大小为[2,100]，得到的输出是(batch_size * doc_len, 100， sent_len-2+1, 1)，
定义一个池化层大小为[sent_len-2+1, 1]，最终得到输出经过squeeze()的形状是(batch_size * doc_len, 100)。
同理，第 2 个卷积核大小为[3,100]，第 3 个卷积核大小为[4,100]。
...
class WordCNNEncoder(nn.Module):
    def __init__(self, vocab):
        ...
        stride：步长。是卷积过程中移动的步长。默认情况下是1。一般卷积核在输入图像上的移动是自左至右，自上至下。如果参数是一个整数那么就默认在水平和垂直方向都是该整数。如果参数是stride = (2, 1), 2
        代表着高（h）进行步长为2，1
        代表着宽（w）进行步长为1。
        padding：填充，默认是0填充。
        dilation：扩张。一般情况下，卷积核与输入图像对应的位置之间的计算是相同尺寸的，也就是说卷积核的大小是3X3，那么它在输入图像上每次作用的区域是3X3，dilation = 0。
        '''
    ...
    def forward(self, word_ids, extword_ids):
        ...
        sen_num, sent_len = word_ids.shape
        # sen_num x sent_len x 100 (100是指100个维度)
        ...
        # 使用unsqueeze函数增加bath_embed的维度
        # 增加维度后的大小: sen_num x 1 x sent_len x 100
        batch_embed.unsqueeze_(1)

        # 通过 3 个卷积核做 3 次卷积核池化
        pooled_outputs = []
        for i in range(len(self.filter_sizes)):
            # sent_len是指句子的长度, +1求区间长度时的植树问题
            # 通过 3 个卷积核做 3 次卷积核池化
            ...
            # 隐藏层的大小: sen_num x out_channel x filter_height x 1
            hidden = F.relu(conv)
            mp = nn.MaxPool2d((filter_height, 1))
            # 得到的mp形式是: (filter_height, filter_width)

            # pooling层把原来的维度从 sen_num x out_channel x 1 x 1 变成了 sen_num x out_channel, 其中out_channel是100维
            pooled = mp(hidden).reshape(sen_num, self.out_channel)
            pooled_outputs.append(pooled)

        # torch.cat是将两个张量（tensor）拼接在一起(concat)
        # reps的维度如下: sen_num x total_out_channel
...
class Model(nn.Module):
    def __init__(self, vocab):
        ...
        # 打印信息: 使用cnn词嵌入作为编码, lstm作为传输的编码器
